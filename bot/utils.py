...
LANGUAGE_MAPPING = {
    ...
    'pt-br': 'en', # Portuguese (Brazil) -> English
...
def normalize_language_code(language_code: Optional[str]) -> str:
    ...
    # Видаляємо регіональні модифікатори якщо вони є
    lang = lang.split('-')[0]
    
    # Повертаємо мапінг або дефолтну мову
    return LANGUAGE_MAPPING.get(lang, DEFAULT_LANGUAGE)
    ...
    try:
        ...
        initial_data = {
            ...
            "statistics": {
                ...
                "languages": {},
                "countries": {},
                "original_languages": {}  # Додано для зберігання оригінальних мов
            },
            "counted_users": [],
            "webapp_opened_users": []  # Змінено для відстеження унікальних відкриттів
        }
        ...
        with open(DATA_FILE, 'r', encoding='utf-8') as file:
            ...
            if "webapp_opened_users" not in data:
                data["webapp_opened_users"] = []
            if "original_languages" not in data.get("statistics", {}):
                data["statistics"]["original_languages"] = {}
...
def validate_data_structure(data: Dict) -> None:
    ...
    required_keys = {
        ...
        "statistics", "counted_users", "webapp_opened_users"
    }
        ...
        if action == 'bot_start':
            if user_id_str not in json_data["counted_users"]:
                ...
                if data and 'language' in data:
                    # Зберігаємо оригінальну мову
                    original_lang = str(data['language']).lower()
                    json_data["statistics"]["original_languages"][original_lang] = \
                        json_data["statistics"]["original_languages"].get(original_lang, 0) + 1
                    
                    # Зберігаємо нормалізовану мову
                    normalized_lang = normalize_language_code(data['language'])
                    json_data["statistics"]["languages"][normalized_lang] = \
                        json_data["statistics"]["languages"].get(normalized_lang, 0) + 1
            ...
            if user_id_str not in json_data["webapp_opened_users"]:
                json_data["statistics"]["webapp_opens"] += 1
                json_data["webapp_opened_users"].append(user_id_str)
...
def get_statistics() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        webapp_opens = len(data.get("webapp_opened_users", []))
        ...
        return {
            ...
            "original_languages": stats.get("original_languages", {}),
...
def delete_user(user_id: int) -> Tuple[bool, str]:
    ...
    try:
        ...
        if user_id_str in data["counted_users"]:
            ...
            if data["statistics"]["total_bot_users"] > 0:
                data["statistics"]["total_bot_users"] -= 1
        
        if user_id_str in data["webapp_opened_users"]:
            data["webapp_opened_users"].remove(user_id_str)
            if data["statistics"]["webapp_opens"] > 0:
                data["statistics"]["webapp_opens"] -= 1
        
        # Оновлюємо мовну статистику
        original_lang = user_data.get("language", "").lower()
        if original_lang in data["statistics"]["original_languages"]:
            if data["statistics"]["original_languages"][original_lang] > 0:
                data["statistics"]["original_languages"][original_lang] -= 1
                if data["statistics"]["original_languages"][original_lang] <= 0:
                    del data["statistics"]["original_languages"][original_lang]
        
        normalized_lang = normalize_language_code(user_data.get("language"))
        if normalized_lang in data["statistics"]["languages"]:
            if data["statistics"]["languages"][normalized_lang] > 0:
                data["statistics"]["languages"][normalized_lang] -= 1
                if data["statistics"]["languages"][normalized_lang] <= 0:
                    del data["statistics"]["languages"][normalized_lang]
...
def register_user(user_id: int, username: str, language_code: str, referral_code: Optional[str] = None) -> Tuple[bool, str]:
    ...
    try:
        ...
        data["users"][user_id_str] = {
            "username": username,
            "language": language_code,  # Оригінальна мова
            "normalized_language": normalized_lang,  # Нормалізована мова
